...
|---|---|---|
| 当前主线 | 当前主线（`tool_count` 必须与 `Tools/spec/tool_catalog.json` 一致） | 必须包含 `rd.vfs.ls` / `rd.vfs.cat` / `rd.vfs.tree` / `rd.vfs.resolve`，并将它们解释为导航层。 |

## 最低对齐要求

- `tool_catalog.snapshot.json` 的 `tool_count` 必须等于 `Tools/spec/tool_catalog.json`。
- `tool_catalog.snapshot.json` 的关键 public contract 也必须反映当前正式 `tool` 语义，不能继续保留旧的返回形态或 mock 成功口径。
- snapshot 中必须包含：
  - `rd.vfs.ls`
  - `rd.vfs.cat`
  - `rd.vfs.tree`
...
- `tabular/tsv` 只能按 projection/support surface 描述，不能写成独立能力面或重要度排序结论。
- `platform_adapter.json` 必须保持 `paths.tools_source_root="tools"` 与 `runtime.mode="worker_staged"`；不再支持手工配置绝对路径、live runtime 目录或占位值替换。
...
- `MCP` opt-in 必须通过 `tools/rdx.bat --non-interactive mcp` 或等价 `cmd /c` 包装接线，不再把系统 `Python` 当作正式前提。
...
- 最新 smoke 报告才是当前 truth；历史 smoke 统计只能当作回归背景，不能再写成固定事实。
