...
`debugger/docs/` 只服务 `debugger/` 维护者，用来说明平台模板、脚手架和目录治理。
...
- `多平台适配说明.md`：说明 `debugger/` 平台模板 contract、`common/` 拷贝工作流，以及 scaffold 生成与校验方法。
- `5分钟接线指南.md`：首次手工接线 `debugger` 到 `RDC-Agent-Tools` 的最短路径。
- `compatibility-matrix.md`：`debugger` 与 `Tools` 的推荐配对方式，以及 snapshot 对齐要求。
