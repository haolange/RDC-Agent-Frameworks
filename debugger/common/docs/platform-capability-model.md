...
- 即便仍标记为 `experimental`，framework 也必须接受当前 `rdx-tools` 的 same-event truthful failure 语义：`rd.shader.debug_start` 不能成功拿到 trace 时，结构化的 blocked/runtime failure 仍属于可依赖行为，不得再把“没有成功 trace”当成“工具没工作”。
