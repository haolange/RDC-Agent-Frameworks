...
- 用户提供可导入的 `.rdc` 后，按 `workflow_stage` 串行推进 specialist 流；当前平台支持 instruction-only sub agents，但不支持实时 team-agent handoff。
- 当前平台的 `sub_agent_mode = instruction_only_sub_agents`；如需 spawn agent，只能由主流程在实例化时注入 instruction，而不是依赖独立 agent 描述文件。
- 当前模板默认不预注册 `MCP`；启用时应显式填写 `.claude/settings.mcp.opt-in.json` 的示例配置。
