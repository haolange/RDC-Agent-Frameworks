...
1. 将 `debugger/common/` 拷贝到 `rdc-debugger/common/`。
2. 将 `RDC-Agent-Tools` 根目录拷贝到 `rdc-debugger/tools/`。
...
- `common/` 与 `tools/` 仍然是用户手动覆盖的 package-local payload。
- 本地插件安装后实际加载的是 cache 副本；每次重新覆盖 `common/` 或 `tools/` 后，都必须重新同步 `~/.agents/plugins/rdc-debugger/`。
