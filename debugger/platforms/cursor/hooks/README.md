...
2. 验证脚本 - 包装 `common/hooks` 中的验证器
3. 工具脚本 - 提供 Cursor 特定的 hook 分发和审计功能
4. Schema 定义 - Cursor 平台特定的验证 schema
...
├── utils/                              # 工具脚本
│   ├── cursor_hook_dispatch.py         # Hook 分发器
...
在 Cursor IDE 或系统环境中设置以下变量：

```bash
export DEBUGGER_ROOT="/path/to/debugger"  # Debugger 框架根目录
export CURSOR_WRITE_PATH=""               # 当前写入的文件路径（由 IDE 设置）
export CURSOR_SESSION_ID=""               # 当前会话 ID（可选）
```
...
BugCard Write -> Tool Contract Validation -> BugCard Schema Validation -> Skeptic Signoff Check
...
1. 路径解析 - 自动解析 `DEBUGGER_ROOT` 环境变量
2. 输出格式化 - 添加 `[cursor-*]` 前缀便于识别
3. 错误处理 - Cursor 特定的错误提示

实际验证逻辑位于：

- `common/hooks/validators/` - 通用验证器
- `common/hooks/utils/` - 通用工具
- `common/hooks/schemas/` - 通用 schema
...
| 1 | 验证失败（有具体错误信息） |
| 2 | 文件/依赖错误（脚本或依赖缺失） |
| 3 | 产物不存在（如 `resolve_session_artifact`） |
...
1. **Cursor 限制** - Cursor 没有原生 lifecycle hooks；`.cursor/rules/rdc-debugger.mdc` 与本目录脚本只是 wrapper 触发面，不是宿主真相。
2. **环境变量** - 确保 `DEBUGGER_ROOT` 环境变量正确设置。
3. **Session 标记** - 某些验证需要 `common/knowledge/library/sessions/.current_session` 文件。
