#!/usr/bin/env python3
"""Counterfactual validator wrapper for Cursor platform.

This script delegates to the common counterfactual_validator.py with
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Counterfactual validator for Cursor platform")
    ...
    common_validator = root / "common" / "hooks" / "validators" / "counterfactual_validator.py"

    if not common_validator.is_file():
        print(f"[cursor-counterfactual] Error: Common validator not found: {common_validator}", file=sys.stderr)
        return 2
